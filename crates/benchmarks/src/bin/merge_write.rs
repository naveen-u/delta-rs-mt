...
fn dummy_array_for_field(
    field: &Field,
    num_rows: usize,
) -> Result<Arc<dyn arrow::array::Array>, Box<dyn Error>> {
    match field.data_type() {
        DataType::UInt32 => {
            let values = vec![0u32; num_rows];
            Ok(Arc::new(UInt32Array::from(values)))
        }
        DataType::Int32 => {
            let values = vec![0i32; num_rows];
            Ok(Arc::new(Int32Array::from(values)))
        }
        DataType::Int64 => {
            let values = vec![0i64; num_rows];
            Ok(Arc::new(Int64Array::from(values)))
        }
        DataType::Utf8 => {
            let values = vec![String::new(); num_rows];
            Ok(Arc::new(StringArray::from(values)))
        }
        // DataType::Decimal128(7, 2) => {
        //     // For Decimal128, we assume a precision of 7 and scale of 2.
        //     // let mut builder = Decimal128Builder::new(num_rows, 7, 2);
        //     // for _ in 0..num_rows {
        ...
        //     // Create a vector of 0i128 values for each row.
        //     let values = vec![0i128; num_rows];
        //     // Build the array using the expected precision and scale.
        //     Ok(Arc::new(Decimal128Array::new(values)))
        // }
        // DataType::Decimal128(7, 2) => {
        //     // Create a Decimal128Builder
        //     let mut builder = Decimal128Builder::new();
...
/// Given a schema and desired number of rows, construct a record batch with dummy data
/// for each field.
fn create_dummy_record_batch(
    ...
    let arrays = schema
        .fields()
        .iter()
        .map(|field| dummy_array_for_field(field, num_rows))
        .collect::<Result<Vec<_>, _>>()?;
    let batch = RecordBatch::try_new(schema, arrays)?;
    Ok(batch)
}

/// Benchmark write-only operations using the WriteFlow (WriteBuilder).
/// This function loads the existing table to determine its schema,
/// creates a dummy record batch that matches the table schema, writes it to the table,
/// and logs the operation’s duration.
