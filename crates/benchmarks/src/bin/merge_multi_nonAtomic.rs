...
/// This function simulates a multi-table transaction by reusing existing
/// single-table benchmark functions. It performs an update on Table 1 and a merge
/// on Table 2 sequentially, and then aggregates the metrics and durations.
async fn benchmark_multi_table_transactions_reuse(
    table1_path: String,
    table2_path: String,
    column: &str,
    multiplier: f64,
    ...
    // Reuse your existing update benchmark on Table 1.
    let (update_duration, update_metrics) =
        benchmark_update_tpcds(table1_path, column, multiplier).await?;

    // Reuse your existing merge benchmark on Table 2.
    // Here we choose, for example, the merge_insert function.
    let (merge_duration, merge_metrics) =
        benchmark_merge_tpcds(table2_path, merge_params, merge_insert).await?;

    let total_duration = Instant::now().duration_since(start);

    // Aggregate metrics from both operations.
    let aggregated_metrics = MultiTableMetrics {
        table1_update_metrics: update_metrics,
        table2_merge_metrics: merge_metrics,
    };

    // (Optional) Log the aggregated metrics to the benchmarks table.
    let schema = Arc::new(ArrowSchema::new(vec![
        Field::new("group_id", DataType::Utf8, false),
        Field::new("name", DataType::Utf8, false),
        Field::new("sample", DataType::UInt32, false),
    ...
    let batch = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(UInt32Array::from(vec![0])),
            Arc::new(UInt32Array::from(vec![total_duration.as_millis() as u32])),
            Arc::new(StringArray::from(vec![json!(aggregated_metrics).to_string()])),
        ],
    )?;
    ...
    println!("Multi-table Transaction Simulation: Total time: {} ms", total_duration.as_millis());
    Ok((total_duration, aggregated_metrics))
}
