...
/// Benchmark multi‑table transactions.
/// This function loads two Delta tables and then begins a single transaction spanning them.
/// It applies an update operation to Table 1 and a merge operation to Table 2,
/// commits the transaction, and logs the aggregated metrics and total duration.
async fn benchmark_multi_table_transactions(
    table1_path: String,
    table2_path: String,
    column: &str,
    multiplier: f64,
    merge_params: MergePerfParams,
) -> Result<(core::time::Duration, MultiTableMetrics), DataFusionError> {
    // Load both tables.
    let table1 = DeltaTableBuilder::from_uri(table1_path).load().await?;
    let table2 = DeltaTableBuilder::from_uri(table2_path).load().await?;

    let start = Instant::now();

    // --- Begin a multi-table transaction ---
    // Here we assume delta‑rs has a new API that allows you to start a transaction
    // spanning multiple tables. For example:
    let mut tx = DeltaOps::begin_transaction(vec![table1.clone(), table2.clone()]).await?;  // replace with what function we hav in DeltaOps for multi-table begin transaction

    // Perform an update on Table 1.
    let (_table1, update_metrics) = tx
        .update_on_table(table1.clone())
        .with_predicate("wr_returned_date_sk IS NOT NULL")
        .with_update(column, format!("{column} * {multiplier}"))
        .await?;

    // For Table 2, we perform a merge.
    // In this example, we simulate a source DataFrame by reading a small sample from Table 2.
    let provider = DeltaTableProvider::try_new(
        table2.snapshot()?.clone(),
        table2.log_store(),
        DeltaScanConfig {
            file_column_name: Some("file_path".to_string()),
            ..Default::default()
    ...
    let ctx = SessionContext::new();
    ctx.register_table("t2", Arc::new(provider))?;
    let df = ctx.sql("SELECT * FROM t2 LIMIT 100").await?;
    let source = df;
    // Use one of your merge functions (here using merge_insert as an example).
    let (_table2, merge_metrics) =
        tx.merge_on_table(table2.clone(), source, merge_insert, merge_params.clone())?.await?;
    ...
    let metrics = MultiTableMetrics {
        table1_update_metrics: update_metrics,
        table2_merge_metrics: merge_metrics,
    };

    // (Optional) Log the multi‑table transaction metrics to the benchmarks table,
    // using a similar approach to the other benchmarks.
    let schema = Arc::new(ArrowSchema::new(vec![
        Field::new("group_id", DataType::Utf8, false),
        Field::new("name", DataType::Utf8, false),
        Field::new("sample", DataType::UInt32, false),
    ...
    let batch = RecordBatch::try_new(
        ...
        vec![
            Arc::new(StringArray::from(vec![group_id])),
            Arc::new(StringArray::from(vec!["multitable-transactions".to_string()])),
            Arc::new(UInt32Array::from(vec![0])),
            Arc::new(UInt32Array::from(vec![duration.as_millis() as u32])),
            Arc::new(StringArray::from(vec![json!(metrics).to_string()])),
        ],
...
#[derive(Debug, serde::Serialize)]
struct MultiTableMetrics {
    table1_update_metrics: UpdateMetrics,
    table2_merge_metrics: MergeMetrics,
}
...
struct MultiTransPerf {
    ...
    sample_matched_rows: f32,
    #[arg(long, default_value = "0.05")]
    sample_not_matched_rows: f32,
}
...
async fn main() {
    ...
    match MergePrefArgs::parse().command {
        ...
        Command::MultiTransPerf(MultiTransPerf { table1, table2, column, multiplier, sample_files, sample_matched_rows, sample_not_matched_rows }) => {
            let merge_params = MergePerfParams {
                sample_files,
                sample_matched_rows,
                sample_not_matched_rows,
            };
            let (duration, metrics) = benchmark_multi_table_transactions_reuse(table1, table2, &column, multiplier, merge_params).await.unwrap();
            println!("Multi-Table Transaction Metrics: {:?}\nTime: {:.2?}", metrics, duration);
        }
    }
}
